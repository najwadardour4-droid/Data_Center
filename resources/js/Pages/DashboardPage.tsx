...
type View = 'dashboard' | 'new-reservation' | 'history' | 'manage-resources' | 'create-resource' | 'edit-resource' | 'categories' | 'approvals' | 'users' | 'logs' | 'settings';

const Dashboard: React.FC = ({ resources, auth, isLoggedIn, reservations, initialView }) => {
    

    const [view, setView] = useState<View>(isLoggedIn && initialView ? initialView : 'manage-resources');
    const [userRole, setUserRole] = useState(isLoggedIn ? auth.role : UserRole.GUEST);
    ...
    const renderContent = () => {
        return (
            ...
            >
                {view === 'dashboard' && <DashboardHome reservations={reservations} resources={resources} auth={auth} onNavigate={setView} userRole={userRole} theme={theme} />}
                ...
                {view === 'history' && <ReservationHistory reservations={reservations} onNew={() => setView('manage-resources')} theme={theme} />}
